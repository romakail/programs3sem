...
int parent (struct pairInfo* pairs, int nChildren)
{
	...
	while (1)
	{
		...
		{
			PRINT ("FD_ISSETRD returned %d\n", FD_ISSET(pairs[i].fdBck[RD], &readFds))
			if (FD_ISSET(pairs[i].fdBck[RD], &readFds))
			{
				readRet = read (pairs[i].fdBck[RD], pairs[i].parentBuffer, pairs[i].parentBufLen);
				PRINT ("parent , read    %d bytes from [%d] child from %d, len = %d\n", readRet, i, pairs[i].fdBck[RD], pairs[i].parentBufLen);
				// {
				// 	printf ("===========//===================\n");
				// 	printf ("i = %d, pairs[i].childNumber = %d\n", i, pairs[i].childNumber);
				// 	printf ("pairs[i].fdBck[RD] = %d\n", pairs[i].fdBck[RD]);
				// 	printf ("pairs[i].parentBuffer = %p\n", pairs[i].parentBuffer);
				// 	printf ("pairs[i].parentBufLen = %d\n", pairs[i].parentBufLen);
				// 	printf ("===========//===================\n");
				// 	exit (-1);
				// }
