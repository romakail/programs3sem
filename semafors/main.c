...
#define SHARED_MEMORY_LENGHT 128
...
int main (int argc, char** argv)
{
    ...
    int semId = semget (SEMAFORES_KEY, NSEMAFORES, IPC_CREAT);
    PRINT ("semId = %d\n", semId)
    if (semId == -1)
    {
    ...
    int shmId = shmget (SHARED_MEMORY_KEY, SHARED_MEMORY_LENGHT, IPC_CREAT | 0644);
    PRINT ("shmId = %d\n", shmId)
    if (shmId == -1)
    {
